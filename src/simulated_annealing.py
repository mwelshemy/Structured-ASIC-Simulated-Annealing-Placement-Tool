...
def build_reverse_lookup():
    global comp_to_nets
    for comp_id in grid_components:
        comp_to_nets[comp_id] = []
        
    for net_idx, net_components in enumerate(grid_nets):
        for comp_id in net_components:
            comp_to_nets[comp_id].append(net_idx)

def calc_net_hpwl(net_components):
    x_coords = []
    y_coords = []
    for comp_id in net_components:
...
def get_total_wire_length():
    ...
    for net in grid_nets:
        total += calc_net_hpwl(net)
    return total

def can_swap(comp_a, comp_b):
    if grid_components[comp_a]['fixed'] or grid_components[comp_b]['fixed']:
        return False
...
def run_simulated_annealing(cooling_rate, components_data, nets_data):
    ...
    final_temp = 5e-6 * (current_cost / num_nets)
    ...
    while current_temp > final_temp:
        ...
        for _ in range(moves_per_temp):
            ...
            if can_swap(comp_a, comp_b):
                affected_nets = set(comp_to_nets[comp_a] + comp_to_nets[comp_b])
                
                old_partial_cost = sum(calc_net_hpwl(grid_nets[idx]) for idx in affected_nets)
                
                swap_cells(comp_a, comp_b)
                
                new_partial_cost = sum(calc_net_hpwl(grid_nets[idx]) for idx in affected_nets)
                
                cost_diff = old_partial_cost - new_partial_cost
